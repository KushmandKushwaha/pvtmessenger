...
import { replyToMessage } from '@/lib/messages/features';
...
export async function POST(request: Request, context: { params: Promise<{ messageId: string }> }) {
  ...
  try {
    ...
    if (!body || typeof body !== 'object' || Array.isArray(body) || typeof body.clientMessageId !== 'string' || typeof body.ciphertext !== 'string' || typeof body.encryptionVersion !== 'number') return NextResponse.json({ error: 'Invalid request.' }, { status: 400 });
    ...
    const result = await replyToMessage(session.userId, session.deviceId, {
      messageId,
    ...
    if (code === 'NOT_AUTHORIZED') return NextResponse.json({ error: 'Message not found.' }, { status: 404 });
    if (code === 'INVALID_INPUT' || code === 'INVALID_CIPHERTEXT') return NextResponse.json({ error: 'Invalid request.' }, { status: 400 });
    logger.error('Message reply failed', { error: code });
    return NextResponse.json({ error: 'Could not create reply.' }, { status: 500 });
  }
}
